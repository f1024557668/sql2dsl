...
import com.wantest.es.sql2dsl.es4sql.domain.hints.HintFactory;
...
public class SqlParser {
    ...
    private void findGroupBy(MySqlSelectQueryBlock query, Select select) throws SqlParseException {
        ...
        for (SQLExpr sqlExpr : items) {
            ...
            if (sqlExpr instanceof SQLSelectGroupByClause) {
                SQLSelectGroupByClause sqlSelectGroupByExpr = (SQLSelectGroupByClause) sqlExpr;
                sqlExpr = sqlSelectGroupByExpr.getItems().get(0);
            }

            if ((sqlExpr instanceof SQLIdentifierExpr || !(sqlExpr instanceof SQLIdentifierExpr || sqlExpr instanceof SQLMethodInvokeExpr)) && !standardGroupBys.isEmpty()) {
